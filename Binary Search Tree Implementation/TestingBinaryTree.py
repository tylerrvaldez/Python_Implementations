...
def main():
    ...
    # clarification statement for testing 
    print(f"\n\n**The expected result of each test is denoted in parenthesis before the actual result**\n\n")

    # Test your method is_similar()
    print(f"TEST: is_similar")
    print(f"tree_a.is_similar(tree_a) => (True) : {tree_a.is_similar(tree_a)}")
    print(f"tree_a.is_similar(tree_b) => (True) : {tree_a.is_similar(tree_b)}")
    print(f"tree_a.is_similar(tree_c) => (False) : {tree_a.is_similar(tree_c)}")
    print(f"tree_a.is_similar(tree_one) => (False) : {tree_a.is_similar(tree_one)}")
    print(f"tree_a.is_similar(tree_zero) => (False) : {tree_a.is_similar(tree_zero)}")
    print(f"tree_one.is_similar(tree_a) => (False) : {tree_one.is_similar(tree_a)}")
    print(f"tree_one.is_similar(tree_one) => (True) : {tree_one.is_similar(tree_one)}")
    print(f"tree_one.is_similar(tree_zero) => (False) : {tree_one.is_similar(tree_zero)}")
    print(f"tree_zero.is_similar(tree_a) => (False) : {tree_zero.is_similar(tree_a)}")
    print(f"tree_zero.is_similar(tree_one) => (False) : {tree_zero.is_similar(tree_one)}")
    print(f"tree_zero.is_similar(tree_zero) => (True) : {tree_zero.is_similar(tree_zero)}\n")
    ...
    print(f"\ntree_c.print_level(-5) => (Nothing) : ", end=" ")
    tree_c.print_level(-5)
    print(f"\ntree_c.print_level(0) => (Nothing) : ", end=" ")
    tree_c.print_level(0)
    print(f"\ntree_c.print_level(1) => (One integer) : ", end=" ")
    tree_c.print_level(1)
    random_level = random.randint(1, tree_c.get_height() + 1)
    print(f"\ntree_c.print_level({random_level}) => (Random integer(s)) : ", end=" ")
    tree_c.print_level(random_level)
    print()

    print(f"\ntree_one.print_level(1) => (One integer) : ", end=" ")
    tree_one.print_level(1)
    print()
    print(f"tree_one.print_level(2) => (Nothing) : ", end=" ")
    tree_one.print_level(2)
    print("\n")

    print(f"tree_zero.print_level(0) => (Nothing) : ", end=" ")
    tree_zero.print_level(0)
    print()
    print(f"tree_zero.print_level(1) => (Nothing) : ", end=" ")
    ...
    print(f"tree_one.get_height() => (0) : {tree_one.get_height()}")
    print(f"tree_zero.get_height() => (-1) : {tree_zero.get_height()}\n")

    # Get the total n
    print(f"TEST: num_nodes")
    print(f"tree_a.num_nodes() => ({size}) : {tree_a.num_nodes()}")
    print(f"tree_b.num_nodes() => ({size}) : {tree_b.num_nodes()}")
    print(f"tree_c.num_nodes() => ({size}) : {tree_c.num_nodes()}")
    print(f"tree_one.num_nodes() => (1) : {tree_one.num_nodes()}")
    print(f"tree_zero.num_nodes() => (0) : {tree_zero.num_nodes()}\n")
